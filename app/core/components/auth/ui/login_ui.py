        ...
        with ui.column().classes('items-center w-full gap-4'):
            ...
            async def try_login():
                ...
                if result:
                    ...
                    ui.notify(t('Willkommen zurück, %{name}!', name=result.full_name), type='positive')
                    ui.navigate.to('/dashboard')
                else:
                    ui.notify(t('Anmeldung fehlgeschlagen: Falscher User oder Passwort'), type='negative')

            ui.button(t('Einloggen'), on_click=try_login).classes('w-full py-4 bg-indigo-600 rounded-xl font-bold')
            ...
            if sso_providers:
                with ui.row().classes('w-full items-center gap-2 my-1'):
                    ...
                    ui.label(t('oder')).classes('text-xs text-zinc-500 uppercase tracking-widest shrink-0')
                    ui.separator().classes('flex-grow bg-zinc-700')
                ...
                for provider in sso_providers:
                    async def sso_login(p=provider):
                        ...
                        if url:
                            ...
                            ui.notify(
                                t('SSO-Anbieter "%{name}" ist derzeit nicht erreichbar.', name=p.display_name),
                                type='warning',
                            )
            ...
            with ui.row().classes('items-center gap-2 opacity-50 mt-2'):
                ui.label(t('Standard: admin / lyndrix')).classes('text-[10px] uppercase tracking-widest')
