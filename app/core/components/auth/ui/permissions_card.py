...
async def render_permissions_card() -> None:
    ...
    with ui.column().classes('w-full gap-4'):
        ...
        with ui.row().classes(
            ...
            ui.label(
                'Berechtigungen werden automatisch aus aktiven Modulen und Plugins geladen. '
    ...
    with ui.expansion(
        ...
        with ui.column().classes('w-full gap-4 pb-2 pt-1 pl-8'):
            ...
            with ui.column().classes('w-full gap-1'):
                ui.label('Gruppen mit dieser Berechtigung').classes(
                ...
                def _render_grp_chips(container=grp_container, perm_id=pdef.id):
                    ...
                    with container:
                        if not _groups:
                            ui.label('Keine Gruppe zugewiesen.').classes(
                ...
                if groups_without:
                    with ui.row().classes('items-center gap-2 mt-1'):
                        ...
                        ).tooltip('Gruppe diese Berechtigung geben')

            # ── Direct user grants ────────────────────────────────────────────
            with ui.column().classes('w-full gap-1'):
                ui.label('Direkte Nutzer-Zuweisungen').classes(
                    'text-[10px] font-bold uppercase tracking-wider text-zinc-500 mb-1'
                )
                ...
                def _render_usr_chips(container=usr_container, perm_id=pdef.id):
                    ...
                    with container:
                        if not _users:
                            ui.label('Keine direkte Zuweisung.').classes(
                                'text-xs text-zinc-600 italic py-1'
                            )
                        for usr in _users:
                            with ui.row().classes(
                ...
                if non_granted_users:
                    with ui.row().classes('items-center gap-2 mt-1'):
                        usr_select = ui.select(
                            ...
                            label='Benutzer direkt hinzufügen',
                        ).props('outlined dark dense').classes('max-w-xs')
                        ...
                        ).tooltip('Direkte Berechtigung vergeben')
