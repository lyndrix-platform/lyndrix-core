    ...
    with ui.column().classes("w-full gap-4"):
            ...
            with ui.row().classes("w-full flex-grow items-center gap-5 p-5"):
                ...
                with ui.column().classes("items-end gap-1 shrink-0"):
                    ...
                    badge_cls = (
                        ...
                        if is_local
                        else "bg-violet-500/15 text-violet-300 border border-violet-500/30"
                    )
        ...
        if is_local:
            ...
            with ui.card().classes(UIStyles.CARD_GLASS + " w-full").style(
                ...
                with ui.row().classes("w-full items-center gap-3 p-5"):
                    ...
                    ui.label(
                        t(
                            'Passwortänderung wird von deinem Auth-Provider "%{provider}" verwaltet. '
        ...
        if extra_perms:
            with ui.card().classes(UIStyles.CARD_GLASS + " w-full").style(
                ...
                ui.element("div").classes(
                    "h-1 w-full bg-gradient-to-r from-teal-400 to-emerald-400"
                )
                with ui.column().classes("w-full flex-grow p-5 gap-2"):
                    with ui.row().classes("items-center gap-2"):
                        ui.icon("verified_user", size="18px").classes("text-teal-400")
                        ui.label(t("Direkt zugewiesene Berechtigungen")).classes(
                            UIStyles.TITLE_H3
                        )
...
def _render_password_change_card(username: str) -> None:
    ...
    with ui.card().classes(
        "w-full bg-zinc-900/60 border border-zinc-800 rounded-2xl"
    ).style("padding: 0; flex-wrap: nowrap"):
        ui.element("div").classes(
            "h-1 w-full bg-gradient-to-r from-sky-400 to-indigo-400"
        )
        with ui.column().classes("w-full flex-grow p-5 gap-3"):
            with ui.row().classes("items-center gap-2 mb-1"):
                ui.icon("lock_reset", size="18px").classes("text-sky-400")
                ui.label(t("Passwort ändern")).classes(
            ...
            if warn:
                with ui.row().classes(
                    ...
                    ui.icon("warning_amber", size="16px").classes(
            ...
            cur_pw = (
                ...
                .props("outlined dark dense autocomplete=current-password")
            ...
            new_pw = (
                ...
                .props("outlined dark dense autocomplete=new-password")
                .classes("w-full max-w-sm")
            )
            conf_pw = (
                ui.input(
                    t("Neues Passwort bestätigen"),
                    password=True,
