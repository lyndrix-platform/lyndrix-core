...
# TODO(agent): expose this aggregate status (subsystem health + CPU/RAM/disk +
# plugin counts) via a dashboard/api.py behind require_api_auth so the React
# frontend can consume the same data instead of reading services directly here.
    ...
    async def dashboard_body():
        with ui.column().classes('w-full max-w-7xl mx-auto gap-8 py-4'):
            ...
            with ui.row().classes('w-full items-center gap-4'):
                ...
                # Re-render the dashboard column in place instead of reloading the page.
                ui.button(icon='refresh', on_click=dashboard_body.refresh).props('flat round color=zinc-500').tooltip('Refresh Dashboard')
